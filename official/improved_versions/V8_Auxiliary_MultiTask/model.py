...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Classifier: Main CTR prediction head
        ...
        # Auxiliary head: Predict interaction type (multi-label auxiliary task)
        # This helps the shared representation learn richer features
        num_aux_tasks = 3  # click, add-to-cart, purchase
        self.aux_head = nn.Sequential(
            nn.Linear(d_model, d_model),
            nn.LayerNorm(d_model),
            nn.SiLU(),
            nn.Dropout(dropout_rate),
            nn.Linear(d_model, num_aux_tasks),
        )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        aux_logits = self.aux_head(output)  # (B, num_aux_tasks)
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        aux_logits = self.aux_head(output)
