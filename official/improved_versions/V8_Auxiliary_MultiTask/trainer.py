...
class PCVRHyFormerRankingTrainer:
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        logits, aux_logits = self.model(model_input)  # (B, 1), (B, num_aux_tasks)
        ...
        # Auxiliary loss: deep supervision on shared representation
        # Use the same label but as multi-label targets (for auxiliary tasks)
        aux_label = label.unsqueeze(1).expand(-1, 3)  # (B, 3)
        aux_loss = F.binary_cross_entropy_with_logits(aux_logits, aux_label) * 0.1
        loss = loss + aux_loss
        loss.backward()
