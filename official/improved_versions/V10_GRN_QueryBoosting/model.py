...
class GRNQueryBoosting(nn.Module):
    """Gated Residual Network for Query Boosting.

    Replaces RankMixer token-mixing with a learnable gated mechanism.
    For each of T tokens, applies independent gating then a shared FFN.

    Structure: x -> GLU(gate(x)) -> FFN -> residual add
    """
    ...
    def __init__(
        ...
        self.ffn_linear1 = nn.Linear(d_model, d_model * hidden_mult)
        self.ffn_linear2 = nn.Linear(d_model * hidden_mult, d_model)
    ...
    def forward(self, Q: torch.Tensor) -> torch.Tensor:
        ...
        # Gating: learn which dimensions to pass through
        gate = torch.sigmoid(self.gate_linear(x))
        x_gated = x * gate

        # Shared FFN
        ffn_out = self.ffn_linear1(x_gated)
        ffn_out = F.gelu(ffn_out)
        ffn_out = self.dropout(ffn_out)
        ffn_out = self.ffn_linear2(ffn_out)

        Q_boost = residual + ffn_out
...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    def __init__(
        ...
        # GRN Query Boosting: replaces RankMixer with gated residual mechanism
        n_total = num_queries * num_sequences + num_ns
        self.mixer = GRNQueryBoosting(
            d_model=d_model,
            n_total=n_total,
            hidden_mult=hidden_mult,
            dropout=dropout,
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # ================== Plot Total T for logging ==================
        T = num_queries * self.num_sequences + self.num_ns
        if T > 0:
            logging.info(f"PCVRHyFormer (GRN): T={T}, num_ns={self.num_ns}, "
                         f"num_queries={num_queries}, num_sequences={self.num_sequences}")
        else:
            raise ValueError(
                f"Invalid topology: T={T}, check num_ns={self.num_ns}, "
                f"num_queries={num_queries}, num_sequences={self.num_sequences}"
            )
