...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Output: Multi-Head Attention Pooling over all query tokens
        self.output_proj = nn.Sequential(
            nn.Linear(num_queries * self.num_sequences * d_model, d_model),
            nn.LayerNorm(d_model),
        )
        # Multi-head attention pooling: learns to aggregate Q tokens adaptively
        self.output_attn_pool = nn.MultiheadAttention(
            embed_dim=d_model,
            num_heads=num_heads,
            dropout=dropout_rate,
            batch_first=True,
        )
        self.output_pool_norm = nn.LayerNorm(d_model)
    ...
    def _run_multi_seq_blocks(
        ...
        # Output: Multi-Head Attention Pooling across Q tokens
        B = curr_qs[0].shape[0]
        all_q = torch.cat(curr_qs, dim=1)  # (B, Nq*S, D)
        output = all_q.view(B, -1)  # (B, Nq*S*D)
        output = self.output_proj(output)  # (B, D)

        # Attention pooling branch: learnable query attends to all concatenated Q tokens
        q_pool = self.output_pool_norm(all_q)  # (B, Nq*S, D)
        attn_output, _ = self.output_attn_pool(
            q_pool.mean(dim=1, keepdim=True),  # (B, 1, D) as query
            q_pool, q_pool,  # (B, Nq*S, D) as key/value
        )
        output = output + attn_output.squeeze(1)  # (B, D), residual fusion
