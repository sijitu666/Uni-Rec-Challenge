...
def asymmetric_focal_loss(
    ...
    pos_gamma: float = 2.0,
    neg_gamma: float = 4.0,
    clip_prob: float = 0.01,
    reduction: str = 'mean',
) -> torch.Tensor:
    """Asymmetric Focal Loss with different gamma for positive/negative samples.

    Uses asymmetric focusing: negative samples get higher gamma to more
    aggressively down-weight easy negatives (class imbalance mitigation).
    Probability clipping prevents log(0) instability.

    Args:
        logits: (N,) raw logits.
        targets: (N,) binary labels {0, 1}.
        pos_alpha: weight for positive class.
        neg_alpha: weight for negative class.
        pos_gamma: focusing for positive samples.
        neg_gamma: focusing for negative samples (higher = more down-weight).
        clip_prob: minimum probability to avoid log(0).
        reduction: 'mean' | 'sum' | 'none'.
    """
    p = torch.sigmoid(logits)
    p = torch.clamp(p, clip_prob, 1.0 - clip_prob)

    pos_mask = targets == 1
    neg_mask = targets == 0

    bce_loss = torch.zeros_like(logits)
    bce_loss[pos_mask] = -torch.log(p[pos_mask])
    bce_loss[neg_mask] = -torch.log(1.0 - p[neg_mask])

    focal_weight = torch.ones_like(logits)
    focal_weight[pos_mask] = (1.0 - p[pos_mask]) ** pos_gamma
    focal_weight[neg_mask] = p[neg_mask] ** neg_gamma

    alpha_weight = torch.ones_like(logits)
    alpha_weight[pos_mask] = pos_alpha
    alpha_weight[neg_mask] = neg_alpha
