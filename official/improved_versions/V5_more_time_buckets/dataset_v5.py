"""PCVR Parquet dataset module (V5: 扩展时间桶数量).

相比基线版本，此版本将时间桶边界从64个扩展到128个，
使NUM_TIME_BUCKETS从65增加到129，实现更精细的时间建模。
"""
...
# V5改进: 时间桶边界从64个扩展到128个
# 在原边界之间插入中间值，实现更精细的时间建模
# 129个桶 = 128个边界 + 1个padding桶
_ORIGINAL_BOUNDARIES = np.array([
...
# 在每对相邻边界之间插入中间值，使边界数量翻倍
def _double_boundaries(boundaries: np.ndarray) -> np.ndarray:
    """在原边界之间插入中间值，使边界数量翻倍。"""
    new_boundaries = []
    ...
    return np.array(new_boundaries, dtype=np.int64)

# V5: 128个边界 -> 129个桶
BUCKET_BOUNDARIES = _double_boundaries(_ORIGINAL_BOUNDARIES)

NUM_TIME_BUCKETS = len(BUCKET_BOUNDARIES) + 1  # 129

logging.info(f"V5 time buckets: {NUM_TIME_BUCKETS} (boundaries: {len(BUCKET_BOUNDARIES)})")
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        logging.info(
            f"PCVRParquetDataset (V5): {self.num_rows} rows, batch_size={batch_size}, "
            f"buffer_batches={buffer_batches}, NUM_TIME_BUCKETS={NUM_TIME_BUCKETS}")
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    logging.info(f"Parquet train: {train_rows} rows, valid: {valid_rows} rows, "
                 ...
                 f"NUM_TIME_BUCKETS={NUM_TIME_BUCKETS}")
