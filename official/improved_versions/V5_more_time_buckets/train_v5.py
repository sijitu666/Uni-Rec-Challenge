"""PCVRHyFormer training entry point (V5: 扩展时间桶版本).

与基线train.py的区别:
- 导入dataset_v5 (NUM_TIME_BUCKETS=129) 而不是 dataset (NUM_TIME_BUCKETS=65)
- 其他逻辑完全相同
"""
...
from dataset_v5 import FeatureSchema, get_pcvr_data, NUM_TIME_BUCKETS
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PCVRHyFormer Training (V5: 129 time buckets)")
...
def main() -> None:
    ...
    logging.info(f"V5: Using NUM_TIME_BUCKETS={NUM_TIME_BUCKETS}")
    ...
    logging.info("Using Parquet data format (IterableDataset) - V5: 129 time buckets")
    ...
    if args.ns_groups_json and os.path.exists(args.ns_groups_json):
        ...
        item_ns_groups = [[item_fid_to_idx[f] for fids in ns_groups_cfg['item_ns_groups'].values()]]
    ...
    logging.info(f"PCVRHyFormer model created (V5): num_ns={num_ns}, T={T}, d_model={args.d_model}")
