...
class SwiGLU_activation(nn.Module):
    """SwiGLU gating activation for use inside nn.Sequential (FFN blocks).

    Input: (B, L, 2*hidden_dim)
...
class SENetRecalibration(nn.Module):
    """Squeeze-and-Excitation feature recalibration for CTR models.

    Learns feature-wise importance weights via squeeze(mean-pool) and
    excitation(two-layer FC with reduction ratio). Applied before NS
    tokenization to emphasize informative features.
    """

    def __init__(self, num_features: int, reduction: int = 4) -> None:
        super().__init__()
        self.num_features = num_features
        reduced = max(1, num_features // reduction)
        self.squeeze = nn.AdaptiveAvgPool1d(1)  # not used, manual mean
        self.fc = nn.Sequential(
            nn.Linear(num_features, reduced),
            nn.SiLU(),
            nn.Linear(reduced, num_features),
            nn.Sigmoid(),
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Applies feature-wise recalibration.

        Args:
            x: (B, num_features, D) feature embeddings.

        Returns:
            Recalibrated tensor of same shape (B, num_features, D).
        """
        B, F, D = x.shape
        z = x.mean(dim=-1)  # (B, num_features), squeeze via mean pooling
        w = self.fc(z)  # (B, num_features), excitation weights
        return x * w.unsqueeze(-1)  # (B, num_features, D)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        self.se_user = SENetRecalibration(num_user_ns) if num_user_ns > 0 else None
        self.se_item = SENetRecalibration(num_item_ns) if num_item_ns > 0 else None
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 1. NS tokens: grouped projection with SENet recalibration
        ...
        if self.se_user is not None:
            user_ns = self.se_user(user_ns)
        if self.se_item is not None:
            item_ns = self.se_item(item_ns)
