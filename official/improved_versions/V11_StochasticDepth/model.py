...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Stochastic depth: linearly increasing survival probability (0.8 -> 1.0)
        self.stoch_depth_probs = [
            0.8 + 0.2 * (i / max(1, num_hyformer_blocks - 1))
            for i in range(num_hyformer_blocks)
        ]
    ...
    def _run_multi_seq_blocks(
        ...
        for idx, block in enumerate(self.blocks):
            ...
            block_qs, block_ns, block_seqs, block_masks = block(
            ...
            if self.training:
                survival_prob = self.stoch_depth_probs[idx]
                if torch.rand(1).item() > survival_prob:
                    continue  # skip this block
                # Scale the block output by 1/survival_prob for expectation
                scale = 1.0 / survival_prob
                block_qs = [curr_q + (bq - curr_q) * scale for curr_q, bq in zip(curr_qs, block_qs)]
                block_ns = curr_ns + (block_ns - curr_ns) * scale
                block_seqs = [curr_s + (bs - curr_s) * scale for curr_s, bs in zip(curr_seqs, block_seqs)]
                # block_masks unchanged

            curr_qs, curr_ns, curr_seqs, curr_masks = block_qs, block_ns, block_seqs, block_masks
