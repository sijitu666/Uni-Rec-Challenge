...
def smooth_labels(labels: torch.Tensor, smoothing: float = 0.1) -> torch.Tensor:
    """Applies label smoothing: y_smooth = y * (1 - smoothing) + 0.5 * smoothing.

    For binary classification, distributes smoothing mass uniformly between
    the two classes (0 and 1).

    Args:
        labels: (N,) or (*) binary labels {0, 1}.
        smoothing: Smoothing factor in (0, 1). 0 = no smoothing.

    Returns:
        Smoothed labels of same shape.
    """
